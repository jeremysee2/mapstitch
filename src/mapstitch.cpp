    ...
    : is_valid(true)
{
    ...
    Ptr<BFMatcher> dematc = makePtr<BFMatcher>(NORM_HAMMING, false);
...
{
    if (H.empty() || H.type() != CV_64F || H.rows != 2 || H.cols != 3)
    {
        std::cerr << "Error: Invalid transformation matrix H" << std::endl;
    ...
    for (size_t i = 0; i < merged_image.size().area(); ++i)
    {
        ...
        else if (image1.data[i] > 200 && image1.data[i] < 210 && (warped_image.data[i] < 200 || warped_image.data[i] > 210))
        {
            merged_image.data[i] = warped_image.data[i];
        }
        else if (warped_image.data[i] > 200 && warped_image.data[i] < 210 && (image1.data[i] < 200 || image1.data[i] > 210))
        {
